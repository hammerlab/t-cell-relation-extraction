...
Lift from: https://raw.githubusercontent.com/nikhilno1/nlp_projects/f5e4ae159970b6fd613d2c2181265db336acc934/pytorch-transformers-extensions/examples/utils_dataset.py
"""
...
class TcreProcessor(DataProcessor):
    ...
    def _create_examples(self, data_dir, set_type):
        """Creates examples for the training and dev sets from single serialize csv data frame"""
        examples = []
        df = pd.read_csv(osp.join(data_dir, 'data.csv'))
        for c in ['split', 'guid', 'label', 'text']:
            if c not in df:
                raise ValueError('Data set does not contain required filed "{}"'.format(c))
        df = df[df['split'] == set_type]
        if len(df) == 0:
            raise ValueError('Found no records for dataset split "{}" (types present = {})'.format(set_type, df['split'].unique()))
        for i, r in df.iterrows():
            guid = "%s-%s" % (set_type, r['guid'])
            label = str(r['label'])
            assert label in ["0", "1"], 'Record {} has label {} (should be "0" or "1")'.format(r, label)
            examples.append(InputExample(guid=guid, text_a=r['text'], label=label))
        return examples
